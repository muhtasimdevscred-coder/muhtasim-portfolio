...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" className="py-xxl bg-surface-container-highest/30 dark:bg-[#161e33]">
      <div className="max-w-container mx-auto px-gutter">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <h2 className="text-headline-md text-on-surface dark:text-white">Selected Works</h2>
          ...
          <div className="flex gap-4 p-1 bg-surface-container dark:bg-white/10 rounded-xl">
              ...
              <button
                ...
                className={`px-6 py-2 rounded-lg text-label-md transition-standard ${
                  filter === tab
                    ...
                    : 'text-on-surface-variant dark:text-gray-300 hover:bg-surface-variant dark:hover:bg-white/10'
                }`}
              >
                {tab}
              </button>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {visible.map((project) => (
            <article
              key={project.title}
              className="portfolio-card group bg-surface-container-lowest dark:bg-[#1a2238] rounded-2xl overflow-hidden border border-outline-variant/20 dark:border-white/10 transition-standard hover:shadow-2xl hover:-translate-y-2"
            >
              ...
              <div className="p-6">
                <span className="text-primary dark:text-inverse-primary text-label-md uppercase tracking-widest">
                  {project.category}
                </span>
                <h3 className="text-headline-sm text-on-surface dark:text-white mt-2">
                  {project.title}
                </h3>
                <p className="text-on-surface-variant dark:text-gray-300 text-body-md mt-4">
                  {project.description}
                </p>
                <a
                  href="#"
                  className="inline-flex items-center gap-2 text-primary dark:text-inverse-primary text-label-md mt-6 group/link"
                >
                  Read More{' '}
